...
#[allow(
    ...
    reason = "tool method doc attributes are runtime MCP descriptions that must stay byte-identical"
)]
#[tool_router]
impl FallowMcp {
    /// Execute a bounded, read-only JavaScript Code Mode snippet against fallow's MCP host API. `code` must be a JavaScript function expression or function body that receives `{ fallow, root }` and returns a JSON-serializable value. The embedded sandbox exposes only a typed `fallow` object with read-only analysis calls: analyze, combined, checkChanged, securityCandidates, findDupes, projectInfo, traceExport, traceFile, impactClosure, traceDependency, traceClone, checkHealth, audit, explain, listBoundaries, featureFlags, impact, checkRuntimeCoverage, getHotPaths, getBlastRadius, getImportance, getCleanupCandidates, plus `fallow.run(tool, params)` for the same allowlist. Similar-code is intentionally excluded because Code Mode is capped at 30 seconds; use the standalone find_similar_code and inspect_similar_code MCP tools, which have dedicated 15-minute timeouts. Mutating fix tools are intentionally not exposed. The sandbox has no filesystem, network, module imports, process, require, Deno, Bun, or shell access, and no dynamic code compilation: eval, Function, and the async and generator function constructors are removed, including the `constructor` route reachable through function prototypes. `root` is injected into host calls that omit params.root. `timeout_ms` caps JS execution and subprocess-backed host calls. In Code Mode, analyze, securityCandidates, findDupes, impactClosure, checkHealth, audit, impact, checkRuntimeCoverage, getHotPaths, getBlastRadius, getImportance, and getCleanupCandidates stay subprocess-backed and are killed on timeout. Every other host call runs in this process, where `timeout_ms` is a response deadline rather than a stop signal: the call returns on time and its analysis may finish afterwards, and while abandoned work is still running the next in-process call falls back to the killable subprocess. `max_output_bytes` caps the total fallow JSON read by host calls and, separately, the serialized snippet result: a larger result is refused with `ok:false`, `truncated:true`, `result_bytes`, and a short `result_preview` in place of the value, so return a projection rather than a whole report; an oversized thrown error message is clamped too, at `max_output_bytes` or 4096 bytes, whichever is larger, and reported with `truncated:true` and `error_bytes`. Host calls refused before dispatch (unknown tool name, malformed params, an output budget with nothing left) run nothing and do not spend the `max_host_calls` budget; `limits.max_rejected_host_calls` bounds how many of them `calls[]` records. Host calls are memoized for the duration of one snippet: repeating the same tool with the same params (key order does not matter) is served from that snippet's own cache, spends no `max_host_calls` slot and no output budget, and still appears in `calls[]` marked `cache_hit: true`; nothing is cached between invocations. The trace itself is bounded at `limits.max_recorded_calls` entries: beyond that the host calls still run and still return, only their trace entries are dropped, and `calls_omitted` reports how many, so a snippet looping one cached call cannot grow the response without limit. `fallow.all(requests)` fans out independent host calls in one go: `requests` is an array of `{ tool, params }` objects whose `tool` is the same wire name `fallow.run` takes, and the return value is an array positionally aligned with it whose elements are `{ ok: true, value }` or `{ ok: false, error }`, so one failing element never hides the others. Every batch element goes through the same tool validation, `root` injection, memoization, shared deadline, and output-byte accounting as a single call, and each element that actually dispatches spends one `max_host_calls` slot. The output budget is shared rather than per element: what is left of `max_output_bytes` is divided evenly across the dispatches a batch makes, so a fan-out reads at most `max_output_bytes` in total. A batch is refused whole, before anything runs, when an element is not an object carrying a `tool` string, when it holds more than `max_host_calls` elements, or when it needs more dispatches than the budget has left. The snippet stays synchronous, with no promises in the sandbox: `fallow.all` blocks until every element has resolved, overlapping the subprocess-backed elements while in-process ones run one at a time, so a fan-out cannot stack analyses that nothing can cancel.
    ...
    /// Get project metadata: active framework plugins, discovered source files, and detected entry points. Useful for understanding how fallow sees the project before running analysis. Supports performance tuning (no_cache, threads).
    #[tool(annotations(read_only_hint = true, open_world_hint = true))]
    ...
    /// Audit changed files for dead code, complexity, duplication, and styling. Purpose-built for reviewing AI-generated code. Combines dead-code + complexity + duplication + styling scoped to changed files and returns a verdict (pass/warn/fail). Auto-detects the base branch if not specified. By default, audit runs the base ref too and gates only findings introduced by the changeset; inherited findings are annotated with introduced=false and counted under attribution. Set gate="all" or audit.gate="all" in config to gate every finding. Returns JSON with verdict, summary counts per category, attribution counts, and full issue details with actions array for remediation guidance. Per-analysis payloads nest under `dead_code`, `duplication`, and `complexity`; the `complexity` block carries the health sub-analysis output (audit JSON has no `health` key). Styling analytics are enabled by default in audit: CSS and CSS-in-JS evidence can add `styling_findings`, `css_analytics`, and `styling_health` inside the nested `complexity` block. Set css_deep=false or pass --no-css-deep to skip project-wide styling reachability while keeping local styling checks; set css_deep=true or pass --css-deep to force it back on when config disables it. Styling actions are report-only and must be verified before editing. Set coverage to an Istanbul coverage-final.json path and coverage_root to an absolute coverage-data path prefix when paths need rebasing for accurate CRAP scoring in the health sub-analysis. The audit `complexity` block includes `coverage_source` on CRAP findings and `summary.coverage_source_consistency` when emitted CRAP source data is uniform or mixed. Set group_by to "owner" (CODEOWNERS), "directory", "package" (workspace), or "section" (GitLab CODEOWNERS `[Section]` headers, with `owners` metadata per group) to partition results. Set dead_code_baseline, health_baseline, and/or dupes_baseline to per-analysis baseline file paths (as saved by `fallow dead-code|health|dupes --save-baseline`) so pre-existing issues on touched files do not dominate the verdict; only new issues not present in the respective baseline contribute. explain_skipped only changes the human-format skipped-default-ignores note (human/markdown CLI output); MCP JSON responses stay clean. Set include_entry_exports=true to also report unused exports in entry files (catches typos in framework exports like `meatdata` vs `metadata`); the CLI flag ORs with the `includeEntryExports` config value. Set runtime_coverage to a V8 coverage directory, V8 JSON file, or Istanbul coverage-final.json to fold runtime-coverage findings into the same audit invocation: agents get the `hot-path-touched` verdict alongside dead-code and complexity in one call (a single local capture is free, continuous or multi-capture monitoring requires an active license; informational verdict, no exit-code change). Set min_invocations_hot=N to tune the runtime-coverage hot-path threshold used by audit (default 100). Complexity findings include Angular `<component>` rollups (synthetic per-component finding folding `worst_class_method + template` complexity); ranking and `--targets` use the rollup over per-function entries so a template-heavy component is surfaced as one unit. When config health.thresholdOverrides is set, audit complexity findings use the resolved local thresholds and the nested `complexity` block includes `threshold_overrides` state, one row per (override, unit, `dimension`) with an `outstanding[]` list naming every dimension on which the finding still fires after the override applied (a ceiling the entry leaves unconfigured, or one it raised to a value the unit still exceeds). Synthetic `<template>` findings cover Angular, Vue, Svelte and Astro markup, not Angular alone, and top-level Svelte `{#snippet}` blocks are scored as their own `<snippet:NAME>` units; template-family units are not scored on the CRAP dimension; `.svelte`, `.vue` and `.astro` template-family findings suppress with `<!-- fallow-ignore-next-line complexity -->` at `placement: "above-template-anchor-line"`. See `check_health` for the `component_rollup` payload shape. When `FALLOW_DIFF_FILE` (path to a unified diff) is set in the agent's process environment, EVERY finding (dead-code, complexity, duplication, boundary, runtime-coverage hot paths) is narrowed to source lines inside an added hunk; project-level findings (unused deps, catalog entries, dependency overrides) bypass the filter because they anchor at fixed `package.json` / `pnpm-workspace.yaml` lines a PR rarely touches. When both `FALLOW_DIFF_FILE` and `FALLOW_CHANGED_SINCE` are set, the diff wins for line-level filtering and `--changed-since` still scopes file discovery. The `runtime_coverage.verdict` also promotes `hot-path-touched` over `cold-code-detected` for PR-review contexts; the full unprioritized signal set is in `runtime_coverage.signals[]`. Use this after generating code to verify quality before committing.
    #[tool(annotations(read_only_hint = true, open_world_hint = true))]
    async fn audit(&self, params: Parameters<AuditParams>) -> Result<CallToolResult, McpError> {
        run_audit(&self.binary, params.0).await
    }

    /// Surface the consequential structural DECISIONS a change embeds, each framed as a judgment question for a human with taste (`fallow decision-surface --format json`). The apex of the review brief and the single call that puts taste-decisions in front of a reviewer; separable and cheap, it runs the same changed-code analysis as the brief, NOT the full project pipeline. Returns `kind: "decision-surface"` with `schema_version`, a ranked `decisions[]` list, an optional `truncated` note, and `signal_count`. Each decision carries a `signal_id` (a deterministic anchor to the fallow-emitted candidate it frames; an agent decision whose `signal_id` fallow did not emit must be REJECTED, this is the anti-hallucination contract), a `category` (EXACTLY the SOLID-3: `coupling-boundary`, `public-api-contract`, `dependency`, nothing else), the framed `question`, the `anchor_file`/`anchor_line`, the `blast` (modules affected beyond the diff) and `consequence` rank, the routed `expert[]` (who to ask) with a `bus_factor_one` flag, and structured `actions[]` (`ask-expert`, `suppress`). The surface is CAPPED to a working-memory-sized handful (4 plus or minus 1, configurable via `max_decisions`, clamped to 3-5); decisions beyond the cap collapse into `truncated`. Every `coupling-boundary` and `public-api-contract` decision is suppressible with a `// fallow-ignore` comment on the anchor file (the `suppress` action carries the paste-ready comment); a `dependency` decision anchors on a `package.json`, which cannot carry a comment, so it has no `suppress` action. Always exits 0 (advisory, never a gate). Use `base` to pick the comparison point (defaults to the git merge-base). Use this to answer "what are the few decisions in this change that actually need human judgment?" rather than scanning the whole diff.
    #[tool(annotations(read_only_hint = true, open_world_hint = true))]
    ...
    /// Return first-class blast-radius context alongside local runtime coverage. Pass `coverage` as a V8 coverage directory, single V8 JSON file, or Istanbul `coverage-final.json`. A single local capture is free and runs without a license; continuous or multi-capture runtime monitoring requires an active license. Returns the standard health JSON; agents should read `runtime_coverage.blast_radius`, which contains stable `fallow:blast:<hash>` IDs, caller counts, traffic-weighted caller reach, and low/medium/high risk bands. This blast-radius signal augments context only: it scores the RISK of changing a function. It MUST NOT gate `safe_to_delete` or a confidence score (a function having callers does not make it 'keep'); only the three-state runtime tracking signal (called / never_called / untracked) can issue a deletion verdict.
    #[tool(annotations(read_only_hint = true, open_world_hint = true))]
    ...
    /// Return cleanup candidates grounded in local runtime coverage. Pass `coverage` as a V8 coverage directory, single V8 JSON file, or Istanbul `coverage-final.json`. A single local capture is free and runs without a license; continuous or multi-capture runtime monitoring requires an active license. Returns the standard health JSON; agents should read `runtime_coverage.findings` for `safe_to_delete`, `review_required`, `low_traffic`, and `coverage_unavailable` verdicts.
    #[tool(annotations(read_only_hint = true, open_world_hint = true))]
