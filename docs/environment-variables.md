...
| `FALLOW_BOT_LOGIN` | Explicitly allowlisted posting username for Fallow-owned finding roots and resolution replies. When set, ownership is narrowed to that exact username (an empty or whitespace-only value matches nobody), including personal-access-token accounts and GitLab project access-token notes with `system: false` and no `author.bot`. When unset, provider-native bot metadata remains the compatibility trust boundary. This is an authorship control, not a lifecycle or deduplication token. | unset | `FALLOW_BOT_LOGIN=fallow-bot` |
